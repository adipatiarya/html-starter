...
const exposePlugin =
  async (plugin) =>
  async (...arguments_) =>
    loadPlugin(plugin, ...arguments_);

const getDefaultPlugins = async () =>
  Promise.all(defaultPlugins.flatMap((plugin) => loadPlugin(plugin)));
  ...
  return gulpPlugin(
    ...
    async (file) => {
      ...
      if (options.verbose) {
        console.log(
          `${PLUGIN_NAME}:`,
          chalk.green('✔ ') + file.relative + chalk.gray(` (${message})`),
        );
      }
...
export const gifsicle = imageminGifsicle;
export const mozjpeg = imageminMozjpeg;
export const optipng = imageminOptipng;
